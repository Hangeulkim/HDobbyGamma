...
3. **마우스 가두기** 탭에서도 실행 중인 프로세스를 선택한 뒤 옵션을 켭니다. 선택한 창이 활성화된 동안에만 마우스를 창의 내부 영역에 가둡니다. 포커스를 잃거나 창이 최소화·종료되면 풀립니다. `Ctrl+Alt+L` 또는 트레이 메뉴는 **이번 활성화에서만** 마우스를 풀며 체크 상태와 저장된 규칙은 유지합니다. 대상 창을 떠났다가 다시 활성화하면 가두기가 재개됩니다. 규칙을 완전히 끄려면 체크를 직접 해제하세요.
...
3. In **Mouse confinement**, choose a running process and enable the option. The cursor is confined to its active window's client area. Focus loss, minimize, and exit release it. Press `Ctrl+Alt+L` or use the tray menu to release it **for the current activation** while keeping the saved rule enabled. Confinement resumes after leaving and reactivating the target window. Uncheck the option to turn off the rule completely.
